...
export default {
  ...
  methods: {
    getAsyncData: debounce(async function (name) {
      ...
      //Set the Modal and Dropdown to the GraphQl result
      this.nameContains = data.nameContains;

      //Split the Url and check for validity
      const apipath = name.split("/");
      const asset_contract_address = apipath[4];
      const token_id = apipath[5];
      if (apipath[4] != undefined) {
        //Call API
        await axios
          .get(
          ...
          //Create array and map the response to the array
          .then((response) => {
            let newArray = [];
            ...
            const apiData = {
              ...
              contractSymbol: response.asset_contract.symbol,
              id: response.token_id,
              metadataImageURI: response.image_preview_url,
              metadataName: response.asset_contract.name,
              tokenId: response.token_id,
            };
            newArray.push(apiData);
